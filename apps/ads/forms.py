...
class ExchangeProposalForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user is not None:
            self.fields['ad_sender'].queryset = self.fields['ad_sender'].queryset.filter(user=self.user)
    ...
    def clean(self):
        ...
        if ad_sender and ad_sender.owner != self.user:
            raise forms.ValidationError("Вы можете отправить предложение только от своего объявления.")
