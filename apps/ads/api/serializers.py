...
class ExchangeProposalSerializer(serializers.ModelSerializer):
    ...
    ad_sender_id = serializers.PrimaryKeyRelatedField(queryset=ItemsModel.objects.all(), write_only=True)
    ad_receiver_id = serializers.PrimaryKeyRelatedField(queryset=ItemsModel.objects.all(), write_only=True)

    class Meta:
        model = ExchangeProposalModel
        fields = ['id', 'ad_sender', 'ad_receiver', 'comment', 'status', 'created_at', 'ad_sender_id', 'ad_receiver_id']
