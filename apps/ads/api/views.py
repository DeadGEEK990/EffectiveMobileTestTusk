...
from .serializers import ItemPreviewSerializer, CategorySerializer, ExchangeProposalSerializer, ItemCreateSerializer
...
class MyExchangeProposalViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        if proposal.ad_sender.user != self.request.user and proposal.ad_receiver.user != self.request.user:
    ...
    def accept(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            # ❗ Отменяем все прочие предложения с этими предметами
    ...
    def decline(self, request, pk=None):
        ...
        if proposal.ad_receiver.user != request.user:
            return Response({'detail': 'Вы не можете отклонить этот запрос'}, status=403)
